...
$(patsubst hashes/%.sha1,$(SOURCES)/%,$(wildcard hashes/musl-1.*.*.sha1)): SITE = $(MUSL_SITE)
$(patsubst hashes/%.sha1,$(SOURCES)/%,$(wildcard hashes/musl-*)): SITE = $(MUSL_SNAPSHOT)
$(patsubst hashes/%.sha1,$(SOURCES)/%,$(wildcard hashes/musl-[0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F]*)): SITE = $(MUSL_SNAPSHOT)
